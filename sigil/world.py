...
if CONSOLE: from hud import Console
        ...
        for event in events:
            ...
            if (event.type == _pygame.KEYDOWN):
                if (
                    ...
                    if _funny: print ("Why'd you quit? :'(")
                    self.event_QUIT()
...
class BaseWorld(KeyManager):
    ...
    def __init__(self,
        ...
        self.paused = 0
        if CONSOLE:
            self._console = Console(screen.Display, bg=background)
    ...
    def drawSprites(self):
        ...
        if CONSOLE:
            if not self._console.active:
                _pygame.display.update(r)
            if self._console.active:
