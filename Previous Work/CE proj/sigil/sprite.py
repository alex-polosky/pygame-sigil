    ...
    def collideGroup(self, group):
        ...
        for s in group.sprites():
            if (s == self):
                ...
                #if (xline == 0):
                #    if (yline > 0):
                #        for r in range(0, yline):
                #            if (
                #                ((self.tempOldPos.y + yline) > s.posInWorld.y)
                #                and
                #                ((self.tempOldPos.right + yline) < s.posInWorld.bottom)
                #            ):
                #                self.onCollide(s)
                #    #if (yline < 0):
                #    #    for r in range(yline, 0):
                #
                #if (xline >= 0):
                #    for x in range(0, xline):
                #        if (yline >= 0):
                #            for y in range(0, yline):
                #                if (pibcollide(x, y, s)):
                #                    self.onCollide(s)
                #                    #continue
                #        elif (yline < 0):
                #            for y in range(yline, 0):
                #                if (pibcollide(x, y, s)):
                #                    self.onCollide(s)
                #                    #continue
                #elif (xline < 0):
                #    for x in range(xline, 0):
                #        if (yline >= 0):
                #            for y in range(0, yline):
                #                if (pibcollide(x, y, s)):
                #                    self.onCollide(s)
                ...
                # CHECK TO SEE IF THE POINT INTERSECTS THE BOX OUT, NOT JUST THE ONE THING
                #if (
                #    (
                #        (self.posInWorld.x >= s.rect.left)
                #        #and
                #        #(
                #        #    (self.posInWorld.y <= s.rect.top)
                #        #    or
                #        #    (self.posInWorld.y >= s.rect.bottom)
                #        #)
                #    )
                #):
                #    self.onCollide(s)
                ...
                #if (
                #    (self.posInWorld.x <= s.rect.right)
                #    and
                #    (self.posInWorld.top <= s.rect.bottom)
                #    and
                #    (self.posInWorld.bottom >= s.rect.top)
                #):
                #    self.onCollide(s)
                #    continue
    ...
    def onCollide(self, sprite):
        if (sprite.collisionType == None):
            ...
            if (self.direction == 'left'):
                self.posInWorld.left = sprite.posInWorld.right
            if (self.direction == 'right'):
                self.posInWorld.right = sprite.posInWorld.left
            if (self.direction == 'up'):
                self.posInWorld.top = sprite.posInWorld.bottom
            if (self.direction == 'down'):
                self.posInWorld.bottom = sprite.posInWorld.top
