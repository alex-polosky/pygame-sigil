...
class ZoomCamera(Camera):
    ...
    def hit(self, sprites):        
        ...
        for x in sprites:
            if (
                #(x.posCamera.right >= self.left)# * self.zoom)
                #(x.posCamera.right >= self.x)
                (x.posInWorld.right >= self.left)# * self.zoom)
                and
                #(x.posCamera.bottom >= self.top)# * self.zoom)
                (x.posInWorld.bottom >= self.top)# * self.zoom)
                and
                #(x.posCamera.left <= self.right)# * self.zoom)
                #(x.posInWorld.left <= self.right)# * self.zoom)
                (x.posCamera.left <= self.w)# * self.zoom)
                and
                #(x.posCamera.top <= self.bottom)# * self.zoom)
                #(x.posInWorld.top <= self.bottom)# * self.zoom)
                (x.posCamera.top <= self.h)# * self.zoom)
               ):
                hits.append(x)
    ...
    def update(self):
        ...
        if (self.FOLLOWING != None):
            ...
            if (self.MOVE_X == 1):
                self.x += self.SPEED_X * self.zoom
            elif (self.MOVE_X == -1):
                self.x -= self.SPEED_X * self.zoom
            elif (self.MOVE_X != 0):
                self.x += self.MOVE_X * self.zoom
            if (self.MOVE_Y == 1):
                self.y += self.SPEED_Y * self.zoom
            elif (self.MOVE_Y == -1):
                self.y -= self.SPEED_Y * self.zoom
            elif (self.MOVE_Y != 0):
                self.y += self.MOVE_Y * self.zoom
        #if (self.PERPETUAL_X != 0):
        #    self.MOVE_X = 0
        #if (self.PERPETUAL_Y != 0):
        #    self.MOVE_Y = 0
    ...
    def zoomUpdate(self):
        if (self.changeZoom != 0):
            if (self.changeZoom == 1):
                self.zoom += self.zoomScale
            elif (self.changeZoom == -1):
                self.zoom -= self.zoomScale
            self.changeZoom = 0
            
            if (self.zoom != self._zoom):
