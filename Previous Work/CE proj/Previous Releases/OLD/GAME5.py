...
def main():
    ...
    # I'm going to blit several lines at every 50 interval
    blacksur = pygame.surface.Surface((WIDTH, HEIGHT))
    blacksur.fill((0, 0, 0))
    for x in range(0, 800, GRID):
        r = pygame.rect.Rect(x-1, 0, 2, 800)
        pygame.draw.rect(blacksur, (20, 255, 255), r)
    for y in range(0, 800, GRID):
        r = pygame.rect.Rect(0, y-1, 800, 2)
        pygame.draw.rect(blacksur, (20, 255, 255), r)
    Screen.blit(blacksur, (0, 0, 800, 600))
    pygame.display.flip()
    ...
    while running:
        pygame.display.set_caption("WSAD Moves\tFPS: %d\tCamera-X: %d\tCamera-Y: %d\tSprites: %d" %(clock.get_fps(),
                                                                                       Camera.x,
                                                                                       Camera.y,
                                                                                       numOfSprites))
        ...
        # draw the sprites
        todraw.add(C.hit())
        rects = todraw.draw(Screen)
        character.draw(Screen)
        pygame.display.update(rects)
        
        if (a == 100):
            os.system('clear')
            f.write('World - x: %d\ty: %d\n' %(character.posInWorld.x, character.posInWorld.y))
